...
'use strict';
define(
	['ojs/ojcore', 'knockout', 'jquery', 'ojL10n!./resources/nls/rich-text-editor-strings', './libs/quill/quill'],
	function (oj, ko, $, componentStrings, Quill) {
		...
		function RichTextEditor(context) {
			...
			self.uploadDisplay = "none";
			self.uploadBlockDisplay = "none";
			...
			context.props.then(function (propertyMap) {
				...
				if (!propertyMap.toolbarOptions == undefined) {
					...
					toolBbarOptions = ["text-sizes", "font", "bold","link", "underline","align", "order", "bullet", "italic", "image"];
				}

				toolBbarOptions.forEach(function (item) {
					switch (item) {
						case "font":
							self.fontDisplay = "inline";
							self.fontBlockDisplay = "";
							break;
						case "text-sizes":
		...
		RichTextEditor.prototype._getUpdatedHTML = function () {
			return $("#editor-container div:first-child").text();
		};
